...
setenv bootargs "earlycon=bcm2835aux,0x3f215040 console=tty1 console=ttyS0,115200 8250.nr_uarts=1 ignore_loglevel keep_bootcon loglevel=8 rdinit=init"
...
if tftp ${kernel_addr_r} Image; then
    if tftp ${fdt_addr_r} bcm2837-rpi-3-b-plus.dtb; then
        if tftp ${ramdisk_addr_r} initramfs.uboot; then
            ...
            echo "!! Init initramfs.uboot not found on TFTP"
        fi
    else
